              ...
              <div className={`p-10 space-y-8 flex-1 overflow-y-auto custom-scrollbar transition-opacity ${isSigned ? 'opacity-40' : 'opacity-100'}`}>
                 <div className="flex justify-between items-start border-b-2 border-slate-900 pb-8">
                    <div>
                       ...
                       <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Solutions de Gouvernance</p>
                    </div>
                    <div className="text-right">
                       <h3 className="text-xl font-black uppercase tracking-widest text-slate-900">Devis Officiel</h3>
                       <p className="text-xs text-slate-500 mt-1">N° ARC-{generatedConfig.id.toString().slice(-6)}</p>
                    </div>
                 </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
           ...
           {versions.length > 0 && (
              <div className="lg:col-span-1 space-y-6">
                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2"><History className="w-4 h-4" /> Historique</p>
                 <div className="space-y-3">
                    {versions.map((v) => (
                       <button key={v.id} onClick={() => setGeneratedConfig(v)} className={`w-full p-4 rounded-2xl border text-left transition-all ${generatedConfig.id === v.id ? 'bg-slate-900 border-slate-700 shadow-xl' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}>
                          ...
                          <p className="text-[11px] font-bold text-slate-400 truncate">{v.name}</p>
                       </button>
                    ))}
                 </div>
              </div>
           ...
           <div className={`${versions.length > 0 ? 'lg:col-span-3' : 'lg:col-span-4'} space-y-6`}>
              {step === 1 ? (
                <Card className="p-12 glass-card border-slate-800 bg-gradient-to-br from-slate-900 via-slate-900 to-purple-900/10">
                   ...
                   <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Décrivez votre centre d'activité..." className="w-full h-48 bg-slate-950 border-2 border-slate-800 rounded-[32px] p-8 text-slate-200 text-lg focus:outline-none focus:border-purple-600 transition-all shadow-inner placeholder:text-slate-700" />
                ...
                <div className="space-y-6 animate-in slide-in-from-right-8 duration-500">
                   <Card className="p-8 glass-card border-slate-800 relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-12">
                         <div className="flex items-center gap-6">
                            ...
                            <div>
                               ...
                               <div className="flex gap-4 mt-2">
                                  <span className="text-[10px] font-black text-slate-500 uppercase flex items-center gap-1"><Database className="w-3 h-3" /> {generatedConfig.category}</span>
                         ...
                         <div className="text-right">
                            <p className="text-[10px] font-black text-slate-500 uppercase mb-1">Investissement</p>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12 border-b border-slate-800 pb-12">
                         <Card className="p-6 bg-slate-900 border-slate-800 text-center">
                            <p className="text-[9px] font-black text-slate-500 uppercase mb-1">ROI Prédit</p>
                            <p className="text-xl font-black text-emerald-500">{generatedConfig.stats.roi.toFixed(0)}%</p>
                         </Card>
                         <Card className="p-6 bg-slate-900 border-slate-800 text-center">
                            <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Status Juridique</p>
                            <Badge variant={isSigned ? "success" : "info"} className="mt-2 text-[10px]">{isSigned ? "CONTRAT SIGNÉ" : "EN ATTENTE"}</Badge>
                         </Card>
                         <Card className="p-6 bg-slate-900 border-slate-800 text-center">
                            <p className="text-[9px] font-black text-slate-500 uppercase mb-1">IA Verdict</p>
