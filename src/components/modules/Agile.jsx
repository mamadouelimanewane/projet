...
const Agile = ({ data, setData }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {data.map(s => (
          <button key={s.id} onClick={() => setSelectedSprint(s.id)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all border ${selectedSprint === s.id ? "bg-indigo-600 border-indigo-500 text-white" : "app-surface2 app-border app-text2 hover:border-slate-500"}`}>
          ...
          <div className="app-surface2 border app-border rounded-xl p-5">
            ...
            <div className="mt-4 space-y-2">
              {sprint.stories.map(st => (
                <div key={st.id} className="flex items-center gap-3 p-3 rounded-xl app-surface3 border app-border2 hover:border-slate-500 transition-all">
                  ...
                  <select value={st.statut} onChange={e => updateStory(st.id, e.target.value)}
                    className="app-surface2 border app-border2 rounded-lg px-2 py-1 text-xs app-text focus:outline-none">
      ...
      <div className="app-surface2 border app-border rounded-xl p-5">
        <h3 className="text-sm font-bold app-text mb-4 uppercase tracking-wider">Vélocité par Sprint</h3>
